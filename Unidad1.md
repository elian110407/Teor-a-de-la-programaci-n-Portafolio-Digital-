...
* Ahora, escribiremos los datos de entrada, usando `"printf()"` (similar a "Escribir") y `"scanf()"` (similar a "Leer"). En el `printf`, usamos `"\n"` para que la función `scanf` se muestre debajo del texto. En la función `scanf`, usamos la **máscara de formato** `"%f"` para indicar que el programa espera un valor de tipo **`float`**. Después de esto, se pone una coma e irá el símbolo **`&`**, que le dice al programa que guarde el valor ingresado por el usuario dentro de la variable definida.
